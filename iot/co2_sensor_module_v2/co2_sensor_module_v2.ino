...
const unsigned long healthcheckInterval = 300000; // 10 Minutes

// Button (for tests only)
const int BUTTON_PIN = 13;
int buttonState = 0;

const int MIN_CYCLES = 20;
// Exponential smoothing (like reference code)
float sens_val = 0;

int reference_resistor_kOhm = 10;

// Convert raw ADC reading to CO PPM
float raw_value_to_CO_ppm(float raw_value, float clean_air_baseline) {
  if (raw_value < 1 || clean_air_baseline < 1) return -1; // Invalid input
  
  // Calculate sensor resistance from raw ADC value
  float sensor_R_kOhm = reference_resistor_kOhm * (1023.0 / raw_value - 1.0);
  
  // Calculate clean air resistance
  float clean_air_R_kOhm = reference_resistor_kOhm * (1023.0 / clean_air_baseline - 1.0);
  
  // Estimate 100ppm resistance (typically 25% of clean air resistance for MQ-7)
  float sensor_100ppm_CO_resistance_kOhm = clean_air_R_kOhm * 0.25;
  
  // Calculate resistance ratio and convert to PPM
  float R_relation = sensor_100ppm_CO_resistance_kOhm / sensor_R_kOhm;
  float CO_ppm = 134.0 * R_relation - 35.0;
...
void send_message(String type, unsigned int currentTime, String additional_info) {
  ...
  // String message = String(type) + String(device_id) + String(currentTime) + String(additional_info);
...
void loop() {
  ...
  if (isMeasuring) {
    ...
    if (currentTime - startTime >= 90000) {
      ...
      float raw_reading = analogRead(A0);
      sens_val = sens_val * 0.999 + raw_reading * 0.001;
      float CO_ppm = raw_value_to_CO_ppm(sens_val);
      sensor_prev_values[array_index] = CO_ppm;
  ...
  // Send alert message if button is pressed
  //buttonState = digitalRead(BUTTON_PIN);
  //Serial.println(buttonState);
  //if (buttonState == HIGH) {
  //  send_message("A", currentTime, "DUMMY");
  //}
}
